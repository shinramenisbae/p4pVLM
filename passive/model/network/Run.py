...
def fetch_data(token, last_timestamp):
    ...
    ).strftime("%Y-%m-%dT%I:%M:%S")
    next_timestamp_query = (
        next_timestamp - timedelta(seconds=CHUNK_DURATION)
    ).strftime("%Y-%m-%dT%I:%M:%S")
    params = {
        "columns": COLUMNS,
        "user_id": user_id,
    ...
    response = requests.get(
        f"{BASE_URL}/research/sensor-data",
    ...
    if new_ppg and is_valid_ppg(new_ppg):
        ...
        if data_hash in processed_hashes:
            ...
            for i in range(len(segments)):
                print(
                    f"Segment {i+1}: Valence = {valence_preds[i].item()}, Arousal = {arousal_preds[i].item()}"
                )
